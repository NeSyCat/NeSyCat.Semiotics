...
import { diagramToPrisma, diagramToPrismaPostgres } from '../../components/editor/export/prisma'
...
// Shared fixture — reused by both the Mongo (diagramToPrisma) and Postgres
// (diagramToPrismaPostgres) describe blocks below, since the two dialects
// differ only in the id line / FK scalar, never in the fixture itself.
const d: Diagram = {
  schemaVersion: 1,
  forms: [
...
describe('diagramToPrismaPostgres', () => {
  const out = diagramToPrismaPostgres(d)

  it('id line: Uuid @id @default(uuid()), no @map', () => {
    expect(out).toMatch(/model User \{[\s\S]*id Uuid @id @default\(uuid\(\)\)/)
    expect(out).not.toContain('ObjectId')
    expect(out).not.toContain('@map(')
  })

  it('relation FK scalar is Uuid', () => {
    expect(out).toContain('authorId Uuid')
    expect(out).toContain('author User @relation(fields: [authorId], references: [id])')
  })

  it('value objects, discriminated unions, and scalars are unchanged from Mongo', () => {
    expect(out).toMatch(/type Address \{[\s\S]*street String[\s\S]*\}/)
  ...
  it('differs from the Mongo output only on the id/FK lines', () => {
    const mongo = diagramToPrisma(d)
    expect(out).not.toEqual(mongo)
    // Every non-id/FK line is identical text between the two dialects.
    const strip = (s: string) => s.split('\n').filter((l) => !/^\s*id (ObjectId|Uuid)|Id (ObjectId|Uuid)/.test(l))
    expect(strip(out)).toEqual(strip(mongo))
  })
})

// Test suite for the unified Shape type (types.ts) — FormKind and
...
describe('unified Shape vocabulary', () => {
  ...
  it("a form of each of the 5 kinds constructs, and its point can take each of the 5 shapes", () => {
    ...
    // other kinds get a fresh point via addPoint.
    ...
    for (const kind of VALID_SHAPES) {
      ...
      expect(form !== undefined && form.kind === kind, `addForm constructs a '${kind}' form`).toBe(true)
      ...
      for (const pshape of VALID_SHAPES) {
        ...
        expect(pt !== undefined && pt.shape === pshape, `a point on a '${kind}' form can take shape '${pshape}' (got ${pt?.shape})`).toBe(true)
      }
    }
  })
})

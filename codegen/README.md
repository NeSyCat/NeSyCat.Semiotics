...
```
diagrams/schema.nesycat.json   (conceptual SOT — drawn in NeSyCat editor)
        │
        ▼  codegen/diagram-to-drizzle.ts
lib/db/schema.ts               (generator output — DO NOT EDIT)
        │
...
`lib/db/schema.ts` carries a DO-NOT-EDIT banner. Any change flows through the diagram.
...
6. Save the diagram JSON to `diagrams/schema.nesycat.json`.
7. `npm run db:diagram && npm run db:generate && npm run db:migrate`.

Never hand-edit `lib/db/schema.ts`. If the generator can't express what you need, extend the generator, not the TS output.
